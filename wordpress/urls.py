...
urlpatterns = patterns('wordpress.views',
    ...
    url(r'^taxonomy/(?P<taxonomy>term|category)/(?P<term>[\w\-\./]+)/$',
        TaxonomyArchive.as_view(), name='wp_taxonomy'),

    url(r'^(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/(?P<post_slug>.+)/(?P<slug>.+)/$',
        'object_attachment', name='wp_object_attachment'),
